...
1. **Rank–abundance plots** (showing the “top clones” and long tail)
2. **Clonal space partitions** (how much is occupied by hyperexpanded, large, or rare clones)
...
=== "R"
    ...
    # Merge with metadata (repertoires table) to get Cluster/Tissue/etc.
    # Note: in the nearest immunarch version we will return metadata by default, so
    # you won't need to do that. Sorry for the temporary inconvenience!
    gene_usage_full <- gene_usage_full |>
      left_join(idata$repertoires, by = "imd_repertoire_id")
...
#### Most overrepresented V gene segments
...
#### Cluster-specific V gene distribution
...
#### CDR3 length distribution for the full dataset
...
=== "R"
    ...
    # Merge with metadata (repertoires) to get tissue/cluster info
    length_stats <- length_stats %>%
      left_join(idata$repertoires, by = "imd_repertoire_id")
    
    # Plot: density of CDR3 lengths by tissue
    ggplot(length_stats, aes(x = seq_len, y = n, color = Tissue)) +
...
#### CDR3 length distribution for the selected clusters
...
=== "R"
    ...
    ggplot(length_stats_sel, aes(x = seq_len, y = n, color = Tissue)) +
      geom_line(stat = "identity", linewidth = 1.2) +
...
##### 1. Visualizing rank–abundance: "How steep is the clonal hierarchy?"

A rank–abundance plot shows, for each sample (or group), how cell counts drop as you go from the most to least abundant clone.
...
=== "R"
    ...
    ggplot(clonal_line, aes(x = index, y = count, color = Tissue, group = repertoire_id)) +
      geom_line(alpha = 0.7) +
      scale_y_log10() +
...
**Interpretation:**
If you see a sharp drop (“hockey stick” curve), a handful of clones dominate—often a hallmark of strong antigen-driven selection.
Flatter curves are typically seen in healthy tissues or naive compartments, reflecting higher diversity.

---

##### 2. Clonal space partitioning: "How much is ‘hyperexpanded’?"

Sometimes, you want to **summarize clonal expansion in a single barplot**, splitting each repertoire into bins by abundance:

* “Hyperexpanded”: clones with ≥1% of all cells
* “Large”: ≥0.1%, etc.
* “Rare”: all the way down to singletons

This lets you instantly compare, for example, **how “oligoclonal” each tissue or cluster is**.
...
=== "R"
    ...
    clonal_prop <- airr_clonality_prop(
      idata,
      # Default bins (can be customized)
      output = "stat"
    ) %>%
      left_join(idata$repertoires, by = "imd_repertoire_id")
...
##### 3. Optional: Clonal rank bins

For fine-grained analysis:
How much of the repertoire is explained by the top 10, top 100, or top 1000 clones?
...
*You can plot this as a heatmap, line plot, or grouped barplot as needed.*
...
**Summary:**
Clonality metrics are powerful for immunology, cancer, and infection research.
Use these tools to *compare expansion* across tissues, track responses, and spot rare or dominant clones—fast and reproducibly.

---

Let me know if you want to add an advanced example (cluster-level clonality, or coloring by clinical metadata)!



### `airr_diversity` — diversity

...


### `airr_public` — compute public receptor indices
