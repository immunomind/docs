...
> Due to Seurat upgrading to 5.3.1, NA values are not displayed correctly.
> Please reach out to Seurat developers if you want DimPlot fixed.
> For now, I provide a quick workaround in the code below. 
...
=== "R"
    ...
    # Do this for compatability with Seurat 5.3.1
...
> Please mind the "Tissue": this column *must* be present in your data in order to create repertoires by it.
...
Both AIRR-C and 10x Genomics formats are supported. Pay attention to the receptor schema, locus column (`locus` or `chain`), and use the correct preset for `make_default_preprocessing()`.
...
=== "R"
    ...
    # Seurat 5.3.1 compatability - don't ask me why, the code worked without this line in 5.3.0...
    sdata@meta.data$clonal_prop_bin[is.na(sdata@meta.data$clonal_prop_bin)] <- "NA"
    ...
    # Same 5.3.1 issue
    sdata@meta.data$clonal_rank_bin[is.na(sdata@meta.data$clonal_rank_bin)] <- "NA"
